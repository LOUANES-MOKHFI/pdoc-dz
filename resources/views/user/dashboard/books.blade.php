    ...
    <div class="card">
      ...
      <div class="card-header row">
        ...
        <div class="col-md-6 col-sm-6 left">
          <h3 class="uppercase">Liste de livres ajoutée</h3>
        </div>
        <div class="col-md-6 col-sm-6 right">
          <h5 class="uppercase">{{count($books)}} Livres partagée</h5>
        </div>
          ...
          <table class="table table-striped">
             <thead>
              <tr class="tr">
                ...
                <td width="200">Auteur(s) </td>
                <td width="320">Action</td>
              <!--td>Enseignant</td-->
              </tr>
             </thead>
             @foreach($books as $document1)
                    <tr>
                        
                        <td>{{$document1->titre}}</td>
                        <td>{{$document1->sujet}}</td>
                        <td>{{$document1->editeur}}</td>
                        <td>
                          @if($document1->confirmation == 1)
                            <a href="" class="btn btn-success" style="width: 150px">Livres Confirmer</a>
                          @elseif($document1->confirmation == 0)
                            <a href="" class="btn btn-warning" style="width: 175px">Livres Non confirmer</a>
                            @endif
                           <a href="{{'/user/book/'.$document1->id}}" class="btn btn-info" style="width: 100px">Modifier</a>
                         </td>
                    </tr>
                @endforeach
            <tbody>
