...
<div class="container" style="margin-bottom: 20px">
  @include('layouts.flash_msg')
    ...
    <h3 class="h3">Tout Les Examens @if(!empty($module)) de module <span style="color: red">{{$module->nom_module}}</span>@endif</h3>

  @if($examens->count() == 0)
    <div class="container">
      <div class="alert alert-warning text-center">
        <p class="text-align" style="font-size:20px">Aucun Examens existe</p>
      </div>
    </div>
  @else
<div class=" row" style="margin-bottom: 40px">
      <form class=" card-sm col-7" action="{{url('search-examens')}}" method="get">
        {{ csrf_field()}}
          <div class=" row no-gutters align-items-center">
             <div class="col">
  <select type="text" class="select2 form-control form-control-lg form-control-borderless"  id="nom_examens" name="nom_examens"  autocomplete="nom_examens" autofocus ></select>
             </div>                 
            <div class="col-auto">
     ...
     <div class="col-2"></div>
             <div class="col-3">
                <a href="{{url('/module/'.$module->id.'/add-examens')}}" class="btn btn-info float-right"><i class="fas fa-plus"></i>Ajouter un examen</a>
             </div>
          @endif
     @endif
</div>
      ...
      @foreach($examens as $examen)
        <div class="col-md-3 col-sm-6">
            <div class="product-grid">
              <h5>{{module($examen->id_module)->nom_module}}</h5>
                <div class="product-image">
                  <a href="{{ asset('examens/'.$examen->examens)}}" target="_blank" class=" elevation-8dp">
                     <img class="pic-1" src="/designe/images/folder.jpg">
                     <img class="pic-2" src="/designe/images/folder-open.jpg">
                    </a>
                </div>
                <div class="product-content">
                    <h3 class="title"><a href="{{url('bibliographie-des-modules/examens/'.$examen->id)}}">{{$examen->nom_examens}}</a></h3>
                    <p class="title"><a href="{{url('bibliographie-des-modules/examens/'.$examen->id)}}">{{$examen->annee}}</a></p>
                </div>
            </div>
            <br>
        </div>
