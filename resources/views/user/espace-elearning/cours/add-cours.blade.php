...
<div class="wrap">               
  <div class="services_grid">
    <div class="container" style="margin-bottom: 20px">
      <h3 class="h3">Ajoute Un Cours dans le Module <span style="color: red">{{$module->nom_module}}</span></h3>
      <div class="row container" >
           <form method="POST" action="{{ url('/add-cours') }}" enctype='multipart/form-data'>
                        @csrf
                        <div class="row">
...
<script type="text/javascript">
 ...
 $('#etablissement').keyup(function(){ 
        var query = $(this).val();
        if(query != '')
        {
         var _token = $('input[name="_token"]').val();
         $.ajax({
          url:"{{ route('etablissment.get') }}",
          method:"POST",
          data:{query:query, _token:_token},
          success:function(data){
           $('#all_etablissemnt_selected').fadeIn();  
                    $('#all_etablissemnt_selected').html(data);
          }
         });
        }
    });
    ...
    $(document).on('click', 'li', function(){  
        $('#etablissement').val($(this).text());  
        $('#all_etablissemnt_selected').fadeOut();  
    });  
