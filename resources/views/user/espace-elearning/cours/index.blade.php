...
<div class="container" style="margin-bottom: 20px">
  @include('layouts.flash_msg')
    ...
    <h3 class="h3">Tout Les Cours  @if(!empty($module)) de module <span style="color: red">{{$module->nom_module}}</span>@endif</h3>
  @if($cours->count() == 0)
    <div class="container">
      <div class="alert alert-warning text-center">
        <p class="text-align" style="font-size:20px">Aucun Cours existe</p>
      </div>
    </div>
  @else
<div class=" row" style="margin-bottom: 40px">
      <form class=" card-sm col-7" action="{{url('search-cours')}}" method="get">
        {{ csrf_field()}}
          <div class=" row no-gutters align-items-center">
             <div class="col">
                <select type="text" class="select2 form-control form-control-lg form-control-borderless"  id="nom_cours" name="nom_cours"  autocomplete="nom_cours" autofocus ></select>
             </div>                 
            <div class="col-auto">
     ...
     <div class="col-2"></div>
             <div class="col-3">
                <a href="{{url('/module/'.$module->id.'/add-cours')}}" class="btn btn-info float-right"><i class="fas fa-plus"></i>Ajouter un Cour</a>
             </div>
          @endif
     @endif
</div>
      ...
      @foreach($cours as $cour)
        <div class="col-md-3 col-sm-6">
            <div class="product-grid">
                 <h5>{{module($cour->id_module)->nom_module}}</h5>
                <div class="product-image">
                  <a href="{{ asset('cours/'.$cour->cours)}}" target="_blank" class=" elevation-8dp">
                     <img class="pic-1" src="/designe/images/folder.jpg">
                     <img class="pic-2" src="/designe/images/folder-open.jpg">
                    </a>
                </div>
                <div class="product-content">
                    <h3 class="title">{{$cour->nom_cours}}</h3>
                     <p class="title">{{$cour->annee}}</p>
                </div>
            </div>
            <br>
        </div>
...
$('#nom_cours').select2({
            placeholder: "Nom des cours",
            minimumInputLength: 2,
            ajax: {
                url: '/cours/find_cours',
                dataType: 'json',
