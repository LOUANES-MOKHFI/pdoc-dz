...
<div class="container" style="margin-bottom: 20px">
  @include('layouts.flash_msg')
    <h3 class="h3">Tout Les Etablissement</h3>

  @if($etablissement->count() == 0)
    <div class="container">
      <div class="alert alert-warning text-center">
        <p class="text-align" style="font-size:20px">Aucun Etablisssemnt existe</p>
      </div>
    </div>
  @else
<div class=" row" style="margin-bottom: 40px">
      <form class=" card-sm col-12" action="{{url('search-etablissement')}}" method="get">
        {{ csrf_field()}}
          <div class=" row no-gutters align-items-center">
             <div class="col">
              <select id="etablissement" type="text1" class="form-control form-control-lg select2" name="etablissement" value="{{ old('etablissement') }}" autocomplete="etablissement" autofocus placeholder="Etablissement"></select>
             </div>                 
            <div class="col-auto">
    ...
    <div class="row container">
      <table class="table table-striped table-bordered table-hover">
        ...
        <tbody>
           @foreach($etablissement as $etablissement1)
           <tr>
          <td>{{$etablissement1->nom_etablissement}}</td>
          <td>{{$etablissement1->type_etablissement}}</td>
          <td><a href="{{url('espace-elearning/'.$etablissement1->id.'/'.$etablissement1->nom_etablissement)}}">Voir les modules</a></td>
        </tr>
          @endforeach
        </tbody>
      </table>
...
<script type="text/javascript">
 $('#etablissement').select2({
            ...
            ajax: {
                url: '/etablissement/find_etablissement',
                dataType: 'json',
